...
if (fixture) {
  ...
  result.faultReproduced = checks.filter((check) => !check.defect && check.name !== 'stat-error').every((check) => check.passed)
    && checks.filter((check) => check.defect).every((check) => !check.passed)
    && result.launches.filter((launch) => launch.name.startsWith('clean-')).every((launch) => launch.passed)
    && result.launches.filter((launch) => launch.name.startsWith('empty-')).every(
      (launch) => launch.exitCode === 1 && !launch.error && /Error executing binary:.*(EFTYPE|ENOEXEC|EINVAL)/i.test(launch.stderr));
}
