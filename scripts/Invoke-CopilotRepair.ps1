...
try {
    ...
    if ($Phase -eq 'Prepare') {
        if ($task.mode -eq 'diagnose') {
            ...
            $report.reason = 'External npm artifact and existing upstream fix: no reviewed Hermes repair adapter. No package or target code executed.'
        } else {
...
"@ $workspace (Join-Path $Output 'copilot.log')
            ...
            $report.reason = 'Copilot diagnosis completed; external artifact reproduction and review of existing upstream PR are required. No code change or PR.'
        } else {
