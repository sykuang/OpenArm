...
function Read-RepairTask([string] $TaskId) {
    ...
    if ($task.id -cne $TaskId -or $task.mode -notin @('diagnose', 'cmake')) {
        throw 'Invalid repair task identity or mode. Only diagnosis or native CMake repair is supported; emulation is out of scope.'
    }
