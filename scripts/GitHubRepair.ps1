...
function Read-RepairTask([string] $TaskId) {
    ...
    if ($task.id -cne $TaskId -or $task.mode -notin @('diagnose', 'cmake', 'npm-wrapper')) { throw 'Invalid repair task identity or mode.' }
    ...
    } elseif ($task.mode -eq 'npm-wrapper') {
        if ($task.repository -cne 'https://github.com/vercel-labs/agent-browser' -or $task.sourceSubdirectory -cne '.' -or
            $task.fork -cne '' -or ($task.allowedFiles -join ',') -cne 'bin/agent-browser.js' -or
            $task.packageVersion -cnotmatch '^\d+\.\d+\.\d+$' -or
            $task.packageSha256 -cnotmatch '^[a-f0-9]{64}$' -or
            $task.packageUrl -cne "https://ms-feed-25.pkgs.visualstudio.com/1es-public/_packaging/npm-public/npm/registry/agent-browser/-/agent-browser-$($task.packageVersion).tgz") {
            throw 'Only the reviewed, hash-pinned agent-browser launcher fixture is supported; publishing is not enabled.'
        }
    ...
    foreach ($path in $task.allowedFiles) {
        if ($task.mode -eq 'npm-wrapper') { continue }
...
function Assert-RepairBundle($Bundle, $Task, [string] $RunId, [string] $WorkflowCommit) {
    if ($RunId -cnotmatch '^[1-9][0-9]*$' -or $WorkflowCommit -cnotmatch '^[a-f0-9]{40}$' -or
        ...
        $Bundle.files -isnot [array] -or $Bundle.files.Count -gt 5 -or $Task.mode -notin @('cmake', 'npm-wrapper')) {
...
function Initialize-RepairInput($Task, [string] $Output) {
    if ($Task.mode -eq 'npm-wrapper') {
        . "$PSScriptRoot\AgentBrowserRepair.ps1"
        Initialize-AgentBrowserInput $Task $Output
        return
    }
...
function Invoke-RepairValidation([string] $InputPath, [string] $Output) {
    if ((Read-Json (Join-Path $InputPath 'config.json')).mode -eq 'npm-wrapper') {
        . "$PSScriptRoot\AgentBrowserRepair.ps1"
        return Invoke-AgentBrowserValidation $InputPath $Output
    }
